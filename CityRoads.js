class CityRoads {
    ...
    ifAlone(x,y){
        ...
        if(this.bfs(x,y) == 1){
            ...
            this.bigPixels[y][x] = this.nearestColor;
        }
    }
    bfs(x,y){
        //console.log(x+" "+y);
        ...
        for(let i=0; i<4;i++){
            ...
            if(this.isInTable(tabX[i],tabY[i]) && this.bigPixels[tabY[i]][tabX[i]] > 3 && this.roads[tabY[i]][tabX[i]] == -1){
                if(this.bigPixels[tabY[i]][tabX[i]] == (this.whichNow + 7) || (this.bigPixels[tabY[i]][tabX[i]] == 4 && this.size != 0)){
    ...
    constructor(bigPixels, cityProperties, radius, dist, middle, citySize){
        ...
        this.roads = Array.from({ length: this.bigPixels.length }, () => Array(this.bigPixels[0].length).fill(-1));
        ...
        while (howMany < this.tabRand.length) {
            ...
            if (!this.tabRand.includes(tmp) && Math.pow(middle.m1 - point.x, 2) + Math.pow(middle.m2 - point.y, 2) < Math.pow(radius, 2)) {
